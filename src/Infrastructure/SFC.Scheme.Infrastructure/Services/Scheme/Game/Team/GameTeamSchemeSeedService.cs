...
public class GameTeamSchemeSeedService(
    IMapper mapper,
    IPublishEndpoint publisher,
    IDateTimeService dateTimeService,
    IMetadataService metadataService,
    IGameTeamSchemeRepository gameTeamSchemeRepository,
    IFormationRepository formationRepository) : IGameTeamSchemeSeedService
{
...
#pragma warning disable CA5394 // Do not use insecure randomness
    ...
    private static readonly List<long> GAME_IDS = [8, 9, 10, 11, 12, 13, 14];
    ...
    public Task<IEnumerable<GameTeamScheme>> GetSeedGameTeamSchemesAsync()
    {
        return _gameTeamSchemeRepository.GetByUserIdsAsync(USER_IDS);
    }

    public async Task SeedGameTeamSchemesAsync(CancellationToken cancellationToken = default)
    {
        IEnumerable<GameTeamScheme> schemes = await CreateSeedGameTeamSchemesAsync().ConfigureAwait(true);

        GameTeamScheme[] seedSchemes = await _gameTeamSchemeRepository.AddRangeIfNotExistsAsync([.. schemes]).ConfigureAwait(true);

        await PublishGameTeamSchemesSeededEventAsync(seedSchemes, cancellationToken).ConfigureAwait(true);

        await _metadataService.CompleteAsync(MetadataServiceEnum.Scheme, MetadataDomainEnum.GameTeamScheme, MetadataTypeEnum.Seed).ConfigureAwait(true);
    }
    ...
    private async Task<IEnumerable<GameTeamScheme>> CreateSeedGameTeamSchemesAsync()
    {
        List<GameTeamScheme> result = [];

        foreach (Guid userId in USER_IDS)
        {
            GameTeamScheme scheme = await BuildGameTeamSchemeAsync(userId).ConfigureAwait(true);
            result.Add(scheme);
        }
    ...
    private async Task<GameTeamScheme> BuildGameTeamSchemeAsync(Guid userId)
    {
        ...
        GameTeamScheme scheme = new()
        {
            ...
            UserId = userId,
            GeneralProfile = new GameTeamSchemeGeneralProfile
            {
                Name = formation.ToString(),
                Comment = "This team scheme is perfect."
            },
            Formation = new GameTeamSchemeFormation
            {
                FormationId = formation,
                TypeId = SchemeTypeEnum.Formation
            },
            TeamId = RandomArrayValue(TEAM_IDS),
            GameId = RandomArrayValue(GAME_IDS)
        };

        BuildGameTeamSchemePlayers(formationEntity!).ForEach(scheme.Formation.Players.Add);

        return scheme;
    }

    private static List<GameTeamSchemeFormationPlayer> BuildGameTeamSchemePlayers(Formation formationEntity)
    {
        IEnumerable<GameTeamSchemeFormationPlayer> result = formationEntity.Values.Select(value => new GameTeamSchemeFormationPlayer
        {
            PlayerId = RandomArrayValue(PLAYER_IDS),
            Position = new GameTeamSchemeFormationPlayerPosition
            {
    ...
    private Task PublishGameTeamSchemesSeededEventAsync(IEnumerable<GameTeamScheme> schemes, CancellationToken cancellationToken = default)
    {
        GameTeamSchemesSeeded @event = _mapper.Map<GameTeamSchemesSeeded>(schemes);
